...
import pytest
from sqlalchemy import create_engine, Column, Integer, BigInteger, DateTime, ForeignKey, select
...
def env():
    ...
    return {"SessionLocal": SessionLocal, "User": User, "StorageUsage": StorageUsage,
           "UsageLog": UsageLog, "service": service}
...
def test_monthly_reset_rolls_over_when_reset_at_has_passed(env):
    ...
    service = QuotaService(env["SessionLocal"], env["User"], env["StorageUsage"],
                           env["UsageLog"], select, now_fn=lambda: fixed_now)
