 ...
 * Prefer `images` for brand marks (avoids SVG→canvas edge cases).
 */
export function IconCloud({
      ...
      if (offCtx) {
        if (images) {
          ...
          img.onload = () => {
            ...
            offCtx.closePath();
            offCtx.clip();
            // Soft white plate so monochrome SVGs stay readable
            offCtx.fillStyle = "#ffffff";
            offCtx.fillRect(0, 0, 40, 40);
            offCtx.drawImage(img, 4, 4, 32, 32);
  ...
  return (
    <div className={className ? `relative inline-block ${className}` : "relative inline-block"}>
      <canvas
        ref={canvasRef}
        width={width}
        height={height}
        ...
        aria-label="Interactive 3D research ecosystem cloud"
        role="img"
      />
      {showControl && (
        <Button
