...
def env(mocker):
    ...
    fake_builder.build.return_value = AssembledPrompt(
        system="You are a research assistant.", persona="", project_context="",
        memory="", rag="Widgets are efficient.", task="Question: are widgets efficient?",
        output_schema="", final="ASSEMBLED FINAL PROMPT TEXT",
        prompt_version_id=5, persona_id=None,
    )
    ...
    app.register_blueprint(create_search_blueprint(
        SessionLocal=SessionLocal, UserFile=UserFile, Chunk=Chunk,
        get_prompt_builder=lambda db: fake_builder,
        model_router=model_router, PromptExecution=PromptExecution,
    ))
    ...
    return {
        ...
        "UserFile": UserFile, "Chunk": Chunk, "PromptExecution": PromptExecution,
