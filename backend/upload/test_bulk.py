...
def test_bulk_upload_success_returns_batch_and_jobs(env):
    ...
    jobs = db.execute(
        select(env["UploadJob"]).where(env["UploadJob"].upload_batch_id == batch.id)
    ).scalars().all()
...
def test_batch_status_reports_progress(env):
    ...
    status_resp = env["client"].get(
        f"/api/uploads/batch/{batch_id}/status", headers=_auth(env["access"])
    )
...
def test_batch_status_not_found_for_other_users_batch(env):
    ...
    status_resp = env["client"].get(
        f"/api/uploads/batch/{batch_id}/status", headers=_auth(other_access)
    )
