"""Discover Europe PMC import — Golden Rule wiring (#25)."""
...
def test_europe_pmc_import_attaches_pdf_and_enqueues(discover_app):
    ...
    body = res.get_json()
    assert body["provider"] == "europe_pmc"
    assert body["pdf_attached"] is True
    assert body["analysis_queued"] is True
    assert body["file"]["external_provider"] == "europe_pmc"
    assert body["file"]["external_item_id"] == "PMC7654321"
    tags = json.loads(db.added[0].tags)
    assert "from-europe-pmc" in tags
    assert enqueued == [(4, body["file"]["id"])]
