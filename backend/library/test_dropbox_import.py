"""Dropbox import — Golden Rule wiring (#27)."""
...
def test_dropbox_import_enqueues_analysis(monkeypatch):
    from backend.library import dropbox as dbx
    ...
    @app.post("/api/library/dropbox/import")
    def dropbox_import():
        ...
        for ext_id in file_ids:
            hit = dbx.download_file("tok", ext_id)
            assert hit
    ...
    with client.session_transaction() as sess:
        sess["user_id"] = 8
    res = client.post("/api/library/dropbox/import", json={"file_ids": ["id:file-1"]})
    assert res.status_code == 201, res.get_data(as_text=True)
    body = res.get_json()
    assert body["created"] == 1
    assert body["analysis_queued"] is True
    assert body["source"] == "dropbox"
    assert enqueued == [(8, body["created_ids"][0])]
    assert db.added[0].external_provider == "dropbox"
    assert "from-dropbox" in json.loads(db.added[0].tags)
